...
import { FreePourCustom } from "./components/FreePourCustom";
...
type Screen =
  ...
  | { kind: "freepour-custom" }
  | { kind: "freepour-custom-play"; numColors: number; numBottles: number; shuffleSteps: number };
...
export default function App() {
  ...
  function customConfigFromScreen(s: Screen & { kind: "freepour-custom-play" }): LevelConfig {
    return {
  ...
  const main =
    screen.kind === "journey" ? (
      ...
      <FreePourPicker
        ...
        onCustom={() => navigate({ kind: "freepour-custom" })}
        onBack={() => navigate({ kind: "level", levelIndex: currentLevelIndex() })}
      />
    ) : screen.kind === "freepour-custom" ? (
      <FreePourCustom
        onPlay={(numColors, numBottles, shuffleSteps) =>
          navigate({ kind: "freepour-custom-play", numColors, numBottles, shuffleSteps })
        }
        onBack={() => navigate({ kind: "freepour-pick" })}
      />
    ) : screen.kind === "freepour-custom-play" ? (
      <GameScreen
        key={`custom-${screen.numColors}-${screen.numBottles}-${screen.shuffleSteps}`}
        mode="endless"
        levelIndex={0}
        freePourTierId={null}
